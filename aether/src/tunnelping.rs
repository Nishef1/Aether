...
fn http_probe_request() -> String {
    // The probe only needs to prove real application traffic crosses the
    // selected tunnel. A product-specific User-Agent would unnecessarily
    // identify Aether to the destination and exit-side infrastructure.
    format!(
